...
/// sender has streamed to it — that is the whole condition, and it is deliberately about a
/// *sender* rather than about coverage.
///
/// The gap the condition is guarding did narrow, which is worth recording so nobody
/// re-argues it from scratch. "Something a real Android encoder does and we have not seen"
/// used to include the two things that make a phone's stream unlike a fixture: adaptive
/// bitrate, and a frame that reconfigures the decoder. Both are now decoded from checked-in
/// vectors and asserted lossless — a quality step every eight encode calls, and the branch
/// that keeps the frame `ldacBT_decode` reports as a failure while having already decoded
/// it. Finding out that a bitrate step is *not* a reconfiguration, and that the two need
/// separate fixtures, was most of that work.
///
/// What is left is what no fixture generated by Sony's own encoder can supply: proof that a
/// phone's encoder and ours agree. That is one Android device and five minutes, and until
/// somebody spends them this stays here.
