# Build Status — begun 2026-07-23, brought current 2026-08-10
...
| `proto-fcast` | **Live, driven by the real senders.** FCast protocol v1-v3 (#241): 4-byte-LE + opcode + JSON framing, per-version opcode legality and outbound translation (a v1 sender gets `{time,state}`, a v2 sender every-field-required, a v3 sender null-omission), the reference receiver's own 3s/6s heartbeat (v2+; v1 has no ping to send), multi-sender `PlayUpdate`/`Initial.playData` sync, event subscriptions (the SDK auto-subscribes `MediaItemEnd` on every connection — pinned in the fixtures), and the playlist owned receiver-side so finishing an item plays the next and `itemIndex` moves. Pure cores replay checked-in transcripts of the real terminal sender (SDK 0.3.0) and of nixpkgs' 2024 pre-SDK client (implicit v1, explicit-null fields); `checks.fcast-vm` drives both real transmitters over a real LAN, discovery through the SDK's own mDNS browse. **And now protocol v4 (#248):** the in-place TLS 1.3 upgrade after the plaintext `Version` exchange, a persistent self-signed identity whose SPKI-SHA256 `fp` senders pin (the reference regenerates its key every boot; ours survives restarts), the FlatBuffers control plane (`fcast-flatbuf` is flatc codegen over the vendored spec schema, quarantining the generated `unsafe` (D58) so `proto-fcast` stays `forbid(unsafe_code)`), a sans-I/O `SessionV4` with packet numbering that counts every packet — fixing the reference's unknown-opcode drift — typed `Error` answers across the whole hostile-input matrix, per-session progress cadence, v4 queue mutation with the reference's exact bounds rules, and the relay classification (sender-driven mutations to others, state broadcasts to all, receiver-driven advances to all). v4 is **armed but not announced by default** (`[fcast] announce_v4`): announcing flips SDK senders onto v4 wholesale, and the TXT `fp`+`v=4` must move with the `Version{4}` answer or the SDK quits either way (fp-without-v4 reads as an insecure downgrade; v4-without-fp gives it nothing to pin). Verified three ways: the captured real-sender v4 transcripts (through actual TLS) replay through the parser; loopback tests run the whole TLS upgrade + relay in-process; and `checks.fcast-v4-vm` drives the real SDK sender pinning our fingerprint plus FUTO's own `fast` conformance driver across a 70-case green manifest. **And the three planes that were deferred at v4's landing are built:** WebRTC screen mirroring (#248) — the sender offers, we answer with host candidates only, and `capabilities.mirroring` is now `backend.is_some()` so a build with no media plane still says false and still refuses typed; the FCompanion resource plane (#249), where an `fcomp://` URL becomes an ordinary `http://` one on the shared host and a ranged GET is answered by reading the sender's own storage back over its control connection in parts; and the rest of the fetch seam beside it — inline content published on that same host (which is what gives a pushed DASH manifest a base URL for its relative segments), playlists fetched when a sender points at one rather than pushing it, and `Play.headers` carried into the fetch as a `MediaRequest` (#251), so an auth-gated source plays instead of failing honestly for a reason we had been given. **Not done:** the 48 excluded conformance cases need a real render pipeline (progress cadence, EOS autoplay, fetch failures — the null bench reports no position and never finishes media), an embedded-track model, image/subtitle rendering (honest capability gaps the `ReceiverIntroduction` states), and playback speed (#250, refused over faked). |
...
| `update` | **The whole self-update path** (#343/#345/#360/D59). Two halves: the release *manifest* — a schema-1 JSON carrying the commit, a monotonic build number, and the artifact's digest — and the agent that acts on it. Authentication is GitHub's build provenance, verified against a trust root that is embedded *and* refreshed over TUF and a workflow identity compiled in (D59) — no signing secret exists anywhere. The schedule is pure (window, idle, phase → wait/check/activate) and asserted in virtual time against the shipped 03:30–05:00 window; the agent is the thin shell that reads one clock, downloads on `spawn_blocking`, and stages under a name the launcher cannot spawn until the tree is complete. Every runtime failure degrades to "keep running what you are running", and a wedge that lasts a week says so in terms a person can act on. A person standing at the panel can also ask for a check now, through a command channel and a `watch` of typed progress the settings screen renders (#360) — the same agent, so a manual request mid-stage is serialised behind it rather than racing it for the staging path. **Not done:** no release has ever been installed by it on the real box (#346). |
...
- `packages.castaway-bt-probe-windows` — the controller bring-up tool (`castaway-bt-probe`) for the
  ...
  `[enable] bluetooth = false` in the box's castaway.toml, because a USB claim is
  exclusive and the receiver would otherwise be holding the radio.
...
- **A release says what it is, and GitHub vouches for it** (#343, D59). Every release
  carries `manifest.json`, attested by `actions/attest-build-provenance` — a Sigstore
  bundle signed by a certificate Fulcio issued to this repository's release workflow. A
  commit sha identifies a tree without ordering two of them, so the manifest's monotonic
  build number (`git rev-list --count`, sound because this repo commits linearly) is what
  lets the panel answer "is this newer than me". There is **no signing secret in the
  repository**: forging a release means a visible commit to `release.yml`, not reading a
  secret every workflow could read.
...
- **The receiver updates itself** (#345). One `releases/latest` call a night inside the
  window; the manifest's provenance is verified before it is parsed, then build number →
...
- **A person can ask** (#360). "Check for updates" in Settings drives the *same* agent —
  one thing fetches, verifies, stages and activates, because the order of operations is
  this crate's security property and two of them would be two chances to disagree about
  it. A manual request skips the window and the idle gate (`Request::Manual` in the pure
  policy, so that claim is a virtual-time assertion rather than prose in an actor) and
  skips nothing else: attestation, build ordering, digest and the #344 VMP refusal all
  still apply, and a `hold` file still outranks the press. Nothing is downloaded before
  the person presses the offer, the download reports bytes against the *signed* manifest's
  size, and `back` does not cancel it — the staged tree is useful either way.
- **The box moves onto it** (#346, open). The tooling is written — `nix run
  .#windows-migrate` — and the acceptance needs the panel.

What is *not* done: **the Authenticode certificate and the EVS account do not exist**
(#348), so releases publish without those two signatures and with a red annotation. The
manifest's own authentication needs no secret at all and works today — the provenance over
the first attested release was verified offline, in Rust, as the fixture this crate's tests
run against. What has never happened is a panel taking a release by itself: that needs the
box (#346).
