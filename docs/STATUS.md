...
`nix build` produces a running binary, and `nix flake check` passes — 17 checks, which
include the VM tests (integration, miracast, bluetooth, gamestream, matter, mixer), the
Android-emulator Bluetooth differential (`android-bt`, #225 — a real phone stack pairs,
registers volume, streams aptX HD, and the waveform is correlated per channel), the hosted
