...
Everything below builds with `cargo build`, passes `cargo nextest run` (**1835 tests** as of
2026-08-05; 2303 with `--all-features`), passes `cargo clippy --all-targets -- -D warnings`,
...
checks, and the four Windows cross-build DLL-closure checks.

**What a green check does and does not mean is recorded in `docs/test-matrix.md`** — a
per-mode audit of what each test actually asserts. The short version: 140 of those tests are
compiled by no check at all, no VM test asserts decoded media content, and several claims in
this file were found stale by that audit and corrected on 2026-08-05.
