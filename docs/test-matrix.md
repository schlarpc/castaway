...
> | nothing in the render path compared a whole frame; D38 defers to golden images that did not exist | `pipeline/tests/golden_scenes.rs` — eight blessed frames, six CPU rasters that need no adapter and two composited through wgpu. Two tolerances, because whole-frame mean alone passed a sixteen-level recolouring of every subtitle. Hardware-vs-lavapipe was measured at ≤1 level, so a blessed image travels | #203 |
> | DLNA had no third-party peer at any tier, and its SCPDs were read back by nothing | `integration-vm` runs Home Assistant's own `dlna_dmr` profile (`async-upnp-client`'s `DmrDevice`) against the receiver: SSDP search, description, **the SCPDs parsed into capability flags**, GENA subscriptions on all three services, transport driven, DIDL round-tripped, sink list read out of the event | #202 |
> | `output_stream`'s two status-reading tests failed intermittently and nobody had sorted them | the harness dropped the render loop — and the tap — before the caller read the status, and the never-joined encode thread then flushed and reported the stream stopped. It holds the loop now, and the teardown is asserted head-on rather than described. Not the placement miss and not starvation: a third thing, which is why exclusivity never helped it | #208 (partial) |
> | `bluetooth-vm` stopped at inquiry, and the receiver it built could not have streamed: no `audio` feature, so no decoders, so an **empty AVDTP endpoint list** | BlueZ's own tools browse our SDP records, pair over SSP, read all four endpoints' capabilities, are refused a mismatched codec, and configure/open/**start** a stream. Then bluetoothd's own A2DP source, PipeWire behind it, plays a chirp at us and every 10 ms of the PCM we recorded is located in what was sent — 80% of windows, against 0-15% for recordings built to be swapped, mono, inverted, resampled or half-rate | #186 |
...
> | the M13 trigger had never fired *for loss*, and could not — `needs_keyframe` was set once when a mirror started and cleared by the first keyframe, and nothing ever set it again | a continuity-counter gap on a video PID re-arms it. That count did not exist: the demuxer dropped the damaged access unit and said nothing, so the session layer had no signal. `miracast-vm` now opens mid-GOP and withholds two datagrams on a schedule, and the rate limiter is asserted from the *source's* own arrival times | #192 |
...
> gap columns above — the drop-late-frames path sheds against the *host's* scheduling
> rather than a decoder's, and `Seek`/`Skip*` and a *successful* `LaunchContent` need a
> browser build. #199 closed with the row above — the two functions that were
...
**2026-08-08, both closed by `checks.android-bt` (#225):** the peer is now the *phone*
stack — Android pairs through its own Settings UI (a uiautomator-tapped consent dialog),
registers VOLUME_CHANGED and hears its INTERIM (#211, as CI), negotiates aptX HD, and the
whole path — real encoder → depacketiser → decoder → mixer → recording — is joined and
asserted at signal level, per channel, with the same correlator as `bluetooth-vm`. The EIR
name is also finally read back off the air: the drive script finds the device row by the
name we advertise. What the check does *not* cover: one image, one Android version, SBC/AAC
untouched on this path (the phone always picks aptX HD), and no BIP cover art yet (#87 —
the emulator makes the per-format matrix automatable; that is the named next slice).
...
| MediaPlayback, TargetNavigator, ApplicationBasic, `LaunchContent` | T0+**T2** | `matter-peer` drives play/pause/stop/play, the target list and a `NavigateTarget`, all seven `ApplicationBasic` attributes, and a search | `Seek`/`SkipForward`/`SkipBackward` and their two out-of-range statuses are still unexecuted, and `LaunchContent` only reaches its *refusal*: a searchable app needs a browser build |
...
| FCompanion resource plane (#249) | T0, runs | `companion` units, `adapter_v4_loopback`, `pushed_content.rs` | the wire format is the spec's own, and the loopback case drives the whole round trip: a v4 sender claims a provider id, plays an `fcomp://` URL, and answers the reads an HTTP GET provokes. The load-time existence check (#336) is covered twice over: a loopback case where the sender answers `NotFound` and hears `ResourceNotFound` back, and the two `cast_companion_missing_*` cases in `checks.fcast-v4-vm`'s manifest, which are the only companion cases a published driver runs. **No published sender has served one** — `checks.fcast-v4-vm` does not push local media, so the reference sender's own `fcomp` *transfer* path is untested against ours |
